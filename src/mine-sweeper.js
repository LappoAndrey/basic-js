...
function minesweeper ( matrix ) {
	let matrixRes = JSON.parse(JSON.stringify(matrix))
		for (let i = 0; i < matrix.length; i ++) {
			for (let j = 0; j < matrix[i].length; j ++) {
				...
				if ( (i - 1) >= 0 && [j + 1] < matrix[i - 1].length && matrix[i - 1][j + 1] === true) {
					count = count + 1
				  }				  
				  ...
				  if ([i + 1] < matrix.length  && [j + 1] < matrix[i + 1].length && matrix[i + 1][j + 1] === true) {
					count = count + 1
				  }
				  matrixRes[i][j] = count
		}	
